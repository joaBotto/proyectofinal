...
const creatingProperty = async (newProperty) => {
    const propertyRegistred = await property.findOne({'address.street':newProperty.address.street})
    if (propertyRegistred) {
        ...
        const propertyCreated = await property.create(newProperty)   
          await users.findByIdAndUpdate(newProperty.owner, { $push: { properties: propertyCreated._id } });
