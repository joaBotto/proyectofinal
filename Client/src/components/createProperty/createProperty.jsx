...
import { loadScript } from "../Filtros/utils";
...
export default function CreateProperty() {
  ...
  const [mapCenter, setMapCenter] = useState({ lat: 0, lng: 0 }); // Estado para el centro del mapa
  ...
  const loadGoogleMapsScript = async () => {
    const googleMapsApiUrl = `https://maps.googleapis.com/maps/api/js?key=AIzaSyBDaeWicvigtP9xPv919E-RNoxfvC-Hqik&libraries=places`;
    await loadScript(googleMapsApiUrl);
  };


  const handleSubmitChange = async (values, { setSubmitting }) => {
  ...
  const handleSelect = async (address) => {
    ...
    try {
      ...
      // Actualiza el centro del mapa con las coordenadas obtenidas
      setMapCenter(latLng);
        ...
        {({ values, isSubmitting, setFieldValue }) => (
          ...
          {/* TITULO DE LA PUBLICACION */}<ToastContainer />
           ...
           {/* DESCRIPCION */}
            ...
            <LoadScript onLoad={loadGoogleMapsScript}>
            <PlacesAutocomplete
              value={address}
